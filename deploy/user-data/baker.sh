...
function deploy_appserver() {
    aws s3 cp --only-show-errors s3://trivialsec-assets/deploy-packages/${COMMON_VERSION}/app-nginx.conf /etc/nginx/nginx.conf
    aws s3 cp --only-show-errors s3://trivialsec-assets/deploy-packages/${COMMON_VERSION}/app.zip /tmp/trivialsec/app.zip
    ...
    unzip -qo /tmp/trivialsec/app.zip -d /tmp/trivialsec
