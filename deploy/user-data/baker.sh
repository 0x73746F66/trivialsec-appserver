...
function deploy_appserver() {
    aws s3 cp --only-show-errors s3://trivialsec-assets/deploy-packages/${COMMON_VERSION}/appserver-nginx.conf /etc/nginx/nginx.conf
    aws s3 cp --only-show-errors s3://trivialsec-assets/deploy-packages/${COMMON_VERSION}/appserver.tar.gz /tmp/trivialsec/appserver.tar.gz
