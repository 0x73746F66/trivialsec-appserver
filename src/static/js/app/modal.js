...
const openModal = async e => {
    ...
    const modal_content = document.querySelector(`.modal-content[data-modal-id="${modal_id}"]`)
    const modal = modal_content.parent('.modal-container')
    for await(const el of document.querySelectorAll('.modal-content')) {
        el.classList.remove('modal-open')
    }
    for await(const el of document.querySelectorAll('.modal-container')) {
        el.classList.remove('modal-open')
    }
    modal.classList.add('modal-open')
    modal_content.classList.add('modal-open')
